// app/tech-invoice/tech-invoice-preview-modal.tsx
"use client";

// Adapted from app/invoice-admin/invoice-preview-modal.tsx.
// The ONLY behavioural change vs admin: step 2 (save) posts to the
// token-gated /api/tech-invoice/create-invoice instead of the role-gated
// /api/invoice-admin/create-invoice. Steps 1 (Xendit) and 3 (email) reuse
// the exact same routes the admin page uses — neither requires a login.
...
// Constants kept in sync with the admin modal / WP page.
const BANK = {
...
export default function TechInvoicePreviewModal({
  token,
  invoiceNumber,
  order,
  report,
  ...
  async function confirmSend() {
    ...
    // ── 1. Xendit invoice (best-effort; falls back to bank transfer) ──
    ...
    // ── 2. Save invoice via the TOKEN-GATED route ────────────────────
    setStep("saving");
    try {
      const payload = {
      ...
      if (json.data?.duplicate) {
        console.warn("create_invoice_by_token returned duplicate:", json.data);
      }
    } catch (e) {
    ...
    // ── 3. Email via GAS proxy (non-fatal on error) ──────────────────
    ...
    try {
      ...
      if (!res.ok) console.warn("Email send returned non-OK:", res.status);
    } catch (e) {
  ...
  return (
    <div className="space-y-4 text-sm">
      {customerEmail ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
          ...
          <span className="opacity-80">
            CC: {CC_EMAIL}
            {bccTechEmails.length > 0 ? ` · BCC: ${bccTechEmails.join(", ")}` : ""}
          </span>
        </div>
      ) : (
      ...
      <div className="border-b border-neutral-200 pb-3">
        ...
        <p className="text-base font-semibold text-neutral-900">{invoiceNumber}</p>
      </div>
      ...
      <div className="space-y-1">
        ...
        {c.name_kanji ? <p className="text-xs text-neutral-600">{c.name_kanji}</p> : null}
        ...
        {reportTechs.length > 0 ? (
          <p className="text-xs text-neutral-500">👷 {reportTechs.join(", ")}</p>
        ) : null}
      </div>
      ...
      <div className="space-y-1.5 pt-2 border-t border-neutral-200">
        ...
        {discount > 0 ? <Row label="Diskon" value={`− ${fmtRp(discount)}`} muted /> : null}
        <Row
          label="Total"
          value={fmtRp(total)}
...
function SuccessView({
  ...
  return (
    <div className="text-center py-4">
      ...
      <h4 className="text-base font-semibold text-neutral-900">Invoice berhasil dikirim</h4>
      ...
      <p className="text-[11px] text-neutral-400 mt-4">Anda dapat menutup halaman ini.</p>
    </div>
  );
}
